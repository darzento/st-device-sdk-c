...
iot_error_t iot_bsp_wifi_init()
{
	...
	IOT_INFO("[esp32c6] iot_bsp_wifi_init");
	...
	IOT_INFO("[esp32c6] iot_bsp_wifi_init done");
...
iot_error_t iot_bsp_wifi_set_mode(iot_wifi_conf *conf)
{
	...
	case IOT_WIFI_MODE_STATION:
		...
		if(mode == WIFI_MODE_AP || mode == WIFI_MODE_APSTA) {
			IOT_INFO("[esp32c6] current mode=%d need to call esp_wifi_stop", mode);
	...
	default:
		IOT_ERROR("esp32c6 cannot support this mode = %d", conf->mode);
